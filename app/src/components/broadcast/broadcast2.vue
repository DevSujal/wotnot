<template>
  <div class="content-section m-8 md:ml-72">
    <div class="flex flex-col md:flex-row justify-between mb-4 border-b pb-5">
      <div>
        ...
        <p class="text-sm md:text-base">
          Send out Broadcast Messages using templates
        </p>
      </div>
    ...
    <div class="flex justify-center items-center mb-6">
      <a
        href="/broadcast/GenerateAiMessage"
        target="_blank"
        class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-purple-600 to-purple-400 text-white font-semibold rounded-lg shadow-lg hover:from-green-700 hover:to-green-500 transition-all duration-200"
      >
        <svg
          class="w-5 h-5 mr-2"
          fill="none"
        ...
        <div class="popup-content custom-scrollbar p-4">
          ...
          >
            ...
            <div class="p-4 bg-[#f5f6fa] mb-4">
              ...
              <div class="mb-2">
                <label for="broadcastName" class="block text-sm font-medium"
                  >Broadcast Name<span class="text-red-800">*</span></label
                >
                <input
                  type="text"
              ...
              <!-- input template -->
              <div class="mb-2">
                <label for="templates" class="block text-sm font-medium"
                  >Choose a template<span class="text-red-800">*</span></label
                >
                ...
                <select
                  ...
                  class="border border-gray-300 rounded px-3 py-2 w-full max-h-[50px]"
                  required
                >
                  <option
                ...
                <div v-if="selectedTemplateHasMedia">
                  ...
                  <input
                    type="file"
                    @change="onFileChange"
                    class="mb-2 w-[60%] mr-1"
                  />
                  <div v-if="uploadedMedia">
                    <p class="text-green-800 font-bold">Uploaded</p>
                  </div>
                  <div v-else>
                    <span
                      class="ml-5 w-5 h-5 border-2 border-blue-500 border-t-transparent rounded-full animate-spin inline-block"
                    ></span>
                  </div>
                </div>
            ...
            <h4 class="text-green-700">
              <b>Who do you want to send it to?</b>
            </h4>
            <p class="text-sm mb-2">
              ...
              <a
                ...
                target="_blank"
                class="text-blue-500"
                ><u>Download sample format for contact upload</u></a
              >
            </p>
            ...
            <div class="p-4 bg-[#f5f6fa] mb-4">
              <div class="mb-2">
                <label for="recipients" class="block text-sm font-medium"
                  >Recipients<span class="text-red-800">*</span></label
                >
                <input
                  type="text"
                  v-model="recipients"
              ...
              <div class="mb-1">
                <label for="csvFile" class="block text-sm font-semibold"
                  >Upload CSV for Contacts:</label
                >
                <input
                  type="file"
            ...
            <div v-if="csvUploaded">
              ...
              <p class="text-sm mb-2">
                Imported contacts are not saved to your contacts directory
              </p>
            </div>
            ...
            <!-- Contacts Filter -->
            <div
              class="flex bg-[#f5f6fa] items-center space-x-2 p-2 justify-between"
            >
              ...
              <button
                @click.prevent="fiterBytTags"
                class="bg-gray-300 hover:bg-gray-400 relative my-2 h-auto w-auto p-1 text-white"
              >
                Apply filter
              </button>
            </div>
                  ...
                  <tr v-for="contact in contacts" :key="contact.id">
                    ...
                    <td class="text-center p-2 md:p-4">
                      <div v-for="(tag, index) in contact.tags" :key="index">
                        <span class="font-bold">{{ tag.key }}:</span>
                        {{ tag.value }}
                      </div>
                    </td>
                  </tr>
            ...
            <h4 class="text-green-700">
              <b>When do you want to send the message ?</b>
            </h4>
            ...
            <div class="p-4 bg-[#f5f6fa] mb-4">
              <p class="text-sm font-semibold mb-1">
                <input
                  ...
                  v-model="isScheduled"
                  :value="false"
                  class="scale-150 text-green-500 m-2 accent-green-700"
                />Send Now
              </p>
              ...
              <p class="text-sm font-semibold mb-1">
                <input
                  ...
                  :value="true"
                  class="scale-150 text-green-500 m-2 accent-green-700"
                  @click="currentDateTime"
                />Schedule
              </p>

              <div v-if="isScheduled" class="flex justify-between">
                <div class="w-[50%]">
                  <label for="scheduleDate" class="block text-sm font-medium"
                    >Date<span class="text-red-800">*</span></label
                  >
                  <input
                    type="date"
                ...
                <div class="w-[50%]">
                  <label for="scheduleTime" class="block text-sm font-medium"
                    >Time<span class="text-red-800">*</span>(GMT +5:30)</label
                  >
                  <input
                    type="time"
    ...
    <div class="p-4 filter-container space-x-2">
      <div class="flex items-center">
        <h3 class="text-xl md:text-2xs mb-2 text-green-700">
          <b>Broadcast List </b>
        </h3>
        <div class="pb-2 pl-2">
          <button
            class="text-green-600 underline hover:text-green-700 hover:bg-transparent"
            @click="fetchBroadcastList(this.filterStatus, 1)"
          >
            <i
              class="bi bi-arrow-clockwise inline-block text-xl"
        ...
        <div class="flex items-center filter-container space-x-2">
          ...
          <div class="w-40px">
            <select
              name=""
              id=""
              @change="filterBroadcastsByStatus"
      ...
      >
        ...
        <tbody>
          ...
          >
              ...
              <div
                :class="{
                  'text-emerald-600 font-semibold':
                    broadcast.status === 'Successful', // Green: Success
                  'text-sky-600 font-semibold':
                    broadcast.status === 'Scheduled', // Blue: Info/Scheduled
                  'text-rose-600 font-semibold':
                    broadcast.status === 'Cancelled', // Red: Danger/Cancelled
                  'text-amber-600 font-semibold':
                    broadcast.status === 'Partially Successful', // Amber: Warning/Partial
            ...
            <td class="p-3 md:p-4 text-center border border-gray-200">
              <button
                class="underline text-gray-800 hover:bg-inherit font-medium"
                @click="
                  (showReportPopup = true), fetchBroadcastReport(broadcast.id)
                "
              >
                View Report
              </button>
    ...
    <div class="flex justify-center mt-4">
      <div
        class="flex items-center space-x-4 bg-white shadow-md rounded-lg px-4 py-2"
      >
        <button
    ...
    <PopUp1
      ...
      @close="(showReportPopup = false), (broadcastReports = [''])"
    >
      ...
      <div class="grid grid-cols-4 gap-4 p-4">
        <!-- Sent -->
        <div
          class="bg-gray-100 p-4 rounded-md shadow-sm flex flex-col items-left border-2 border-green-600"
        >
          <div class="flex justify-between items-center">
            <span class="text-3xl font-semibold">{{ totals.sent }}</span>
            <div
              class="w-10 h-10 flex items-center justify-center mb-auto rounded-full bg-white"
            >
              <svg
                width="18"
                height="14"
              ...
              >
                <path
                  d="M1.66797 7L7.16797 12.5L16.3346 1.5"
                  stroke="#23A455"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
        ...
        >
          <div class="flex justify-between items-center">
            ...
            >
              ...
              >
                <path
                  d="M1.83203 6L6.64453 10.5833L9.05078 7.83333"
                  stroke="#23A455"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                ></path>
                <path
                  d="M7.33203 5.99984L12.1445 10.5832L20.1654 1.4165"
                  stroke="#23A455"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                ></path>
                <path
                  d="M14.6654 1.4165L11.457 5.08317"
                  stroke="#23A455"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
      ...
      <div class="p-4 overflow-x-auto max-h-[51vh] custom-scrollbar">
        ...
        >
          ...
          <tbody class="bg-white">
            <tr
              v-for="(contactReport, index) in broadcastReports"
              :key="contactReport.id"
            >
              <td class="border-[#ddd] p-2 md:p-4 text-center">
                {{ contactReport.contact_name }}
              </td>
              <td class="border-[#ddd] p-2 md:p-4 text-center">
                {{ contactReport.phone_no }}
              </td>
              <td class="border-[#ddd] p-2 md:p-4 text-center">
                {{ contactReport.status }}
              </td>
...
export default {
  ...
  <img src="${component.example.header_handle[0]}" alt="Description of image" 
    ...
    async onTemplateSelect() {
      ...
      this.previewData = this.generateTemplatePreview(
      ...
      if (headerComponent) {
        ...
        if (this.imageUrl) {
          try {
            ...
            const file = new File([blob], `template-media.${ext}`, {
              type: contentType,
            });
            ...
            console.log(
              `Template media downloaded and uploaded successfully.: ${this.mediaId}`
            );
          } catch (error) {
    ...
    async fetchContacts() {
      try {
        ...
        const response = await fetch(
          `${this.apiUrl}/contacts/?limit=1000&offset=0`,
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    async fetchBroadcastList(statusFilter = null, page = 1) {
      ...
      const url = `${this.apiUrl}/broadcast?limit=${itemsPerPage}&offset=${
        (page - 1) * itemsPerPage
    ...
    async fetchBroadcastReport(broadcast_id) {
      ...
      try {
        const response = await fetch(
          `${this.apiUrl}/broadcast-report/${broadcast_id}`,
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    async scheduleBroadcast2() {
      ...
      try {
        ...
        const response = await fetch(
          `${this.apiUrl}/schedule-template-message/`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    clearForm() {
      this.popupLoading = false;
      (this.isSubmitted = false), (this.contact = ""), (this.previewData = "");
      (this.broadcastName = ""),
    ...
    async importCSV() {
      ...
      try {
        const response = await axios.post(
          `${this.apiUrl}/contacts/bulk_import/`,
          formData,
          {
            headers: {
              "Content-Type": "multipart/form-data",
    ...
    async fiterBytTags() {
      ...
      try {
        const response = await fetch(
          `${this.apiUrl}/contacts-filter/filter?tag_key=${tagKey}&tag_value=${tagValue}`,
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    selectAll(event) {
      ...
      if (event.target.checked) {
        ...
        this.selectedContacts = this.contacts.map(
